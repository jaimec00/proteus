...
class Hydra(HydraConf):
  run: RunDir = field(default_factory=lambda: RunDir("/home/ubuntu/proteus/experiments/run/${now:%Y-%m-%d}/${now:%H-%M-%S}"))
  sweep: SweepDir = field(default_factory=lambda: SweepDir(
    dir = "/home/ubuntu/proteus/experiments/sweep/${now:%Y-%m-%d}/${now:%H-%M-%S}",
